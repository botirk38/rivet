...
export async function initCommand() {
	try {
		...
		if (error && typeof error === "object" && "isTtyError" in error) {
			console.error(
				chalk.red("Interactive mode not available. Use environment variables instead."),
			);
		} else {
			...
			if (
				...
				"conversation" in error &&
				Array.isArray((error as any).conversation)
			) {
				...
				console.error(
					...
					JSON.stringify(
						convo.map((t: any) => ({
							type: t.type,
							stepsCount: t.type === "agent" ? t.steps?.length : 0,
						})),
						null,
						2,
					),
